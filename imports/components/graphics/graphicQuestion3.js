...
import d3 from 'd3';
import './graphicQuestion3.html';

const name = 'graphicQuestion3';

class GraphicQuestion3Ctrl {
...
//    $scope.$parent.setHeaderFab(false);
   ...
   this.helpers({
    ...
    burbleSiConfig.waveAnimateTime = 1000;
    burbleSiConfig.textSize = 0.80;
    var burbleSi = loadLiquidFillGauge("burbleSi", 0, 0, burbleSiConfig);
    ...
    burbleNoConfig.waveAnimate = true;
    burbleNoConfig.waveRise = false;
    burbleNoConfig.waveHeightScaling = false;
    burbleNoConfig.waveOffset = 0.25;
    burbleNoConfig.textSize = 0.80;
//    burbleNoConfig.waveCount = 3;
    var burbleNo = loadLiquidFillGauge("burbleNo", 0, 0, burbleNoConfig);
...
/************* ACTUALIZACION REAL TIME DE GRAFICOS ***************/
    this.autorun(() => {
      ...
      Meteor.call('answerGroupCount', 3, function(error, result) {

        if (result.length > 0) {
          var wordsAnswerResult = new Object(), total = 0;
...
//          console.log('CANTIDAD NO: ', Math.round((100.0*) / total));
          burbleNo.update(Math.round((100.0*totalNo) / total), totalNo)
            //updateValue(Math.round((100.0*wordsAnswerResult['Si']) / total));
//            $scope.value = Math.round((100.0*wordsAnswerResult['Si']) / total) ;
//          }
        }
...
//  'angularD3LiquidFillGauge',
  ...
  .component(name, {
    templateUrl: 'imports/components/graphics/graphicQuestion3.html',
    controllerAs: name,
    controller: GraphicQuestion3Ctrl
  })
  .config(config);
...
function config($stateProvider, $compileProvider, $logProvider) {
  ...
  $stateProvider.state('graphic-question-3', {
    url: '/',
    template: '<graphic-question-3></graphic-question-3>'
  });
}
...
function loadLiquidFillGauge(elementId, value, valueNumber, config) {
    ...
    var textFinalValueNumber = parseFloat(valueNumber).toFixed(2);

    var textStartValue = config.valueCountUp?config.minValue:textFinalValue;
    var textStartValueNumber = config.valueCountUp?0:textFinalValueNumber;
    ...
    var text1Footer = svgFooter.append("text")
...
//        .attr("dy", "1em")
    ...
    if(config.valueCountUp){
        ...
        text1Footer.transition()
            .duration(config.waveRiseTime)
            .tween("text", textTweenNumber);
        text2.transition()
            .duration(config.waveRiseTime)
            .tween("text", textTween);
    }
    ...
    function GaugeUpdater(){
        this.update = function(value, valueNumber){
